...
PlateReissnerMindlin1p<T>::constitutive_matrix(const LocalFrame<T, 2>& local,
                                               Index q) const
{
    // D = [ D_b   0   ]
    //     [ 0     D_s ]
                                         ...
                                         const LocalFrame<T, 2>& local) const
{
    ...
    for (Index q = 0; q < Q; ++q)
    {
        ...
        B.row(5 * q    ) = -(basis.N_uu.row(q) - Gam1_11 * basis.N_u.row(q) - Gam2_11 * basis.N_v.row(q));
        B.row(5 * q + 1) = -(basis.N_vv.row(q) - Gam1_22 * basis.N_u.row(q) - Gam2_22 * basis.N_v.row(q));
        B.row(5 * q + 2) = -T(2) * (basis.N_uv.row(q) - Gam1_12 * basis.N_u.row(q) - Gam2_12 * basis.N_v.row(q));
