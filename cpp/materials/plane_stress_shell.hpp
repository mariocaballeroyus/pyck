...
public:
     ...
     *
     * Returns a stack-allocated `Eigen::Matrix<T, 3, 3>`. Public so that hot
     * callers (shell stiffness assembly) can compute H once per quadrature
     * point and scale it for membrane (t·H) and bending (t³/12·H), avoiding
     * the redundant build inside `membrane_voigt`/`bending_voigt`.
     */
    Eigen::Matrix<T, 3, 3> surface_H_voigt(const Eigen::Matrix<T, 3, 1>& g_inv) const
    {
        const T g11 = g_inv(0);
        const T g12 = g_inv(1);
        const T g22 = g_inv(2);
