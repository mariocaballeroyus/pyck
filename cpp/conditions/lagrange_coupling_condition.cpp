    ...
    const std::vector<DofLayout::BlockId>& primal_blocks) const
{
    ...
    const auto& parent_a = *side_a_.parent();
    const auto& parent_b = *side_b_.parent();
    ...
    for (Index span_a = 0; span_a < num_spans; ++span_a)
    {
        ...
        // Side A: shape functions, parent lift, parent shape functions.
        auto [derivs_a, jac_a] = side_a_.eval_shape_functions(pts_a, span_a, 1);
        const Index flat_a = side_a_.parent_flat_span(span_a);
        const ColMatrix<T, 2> par_pts_a = side_a_.lift_to_parent(pts_a);
        auto [parent_derivs_a, _ja] =
            parent_a.eval_shape_functions(par_pts_a, flat_a, req_order_a);
        ...
        auto [derivs_b, jac_b] = side_b_.eval_shape_functions(pts_b, span_b, 1);
        (void)jac_b;  // jac_a is the surface measure dΓ; jac_b agrees up to round-off.
        const Index flat_b = side_b_.parent_flat_span(span_b);
        const ColMatrix<T, 2> par_pts_b = side_b_.lift_to_parent(pts_b);
        auto [parent_derivs_b, _jb] =
            parent_b.eval_shape_functions(par_pts_b, flat_b, req_order_b);
        ...
        for (const auto& term : terms_)
        {
            Matrix<T> C_a = term.field_a->evaluate(
                element_a_, side_a_, span_a, derivs_a, flat_a, parent_derivs_a);
            Matrix<T> C_b = term.field_b->evaluate(
                element_b_, side_b_, span_b, derivs_b, flat_b, parent_derivs_b);
            ...
            for (Index q = 0; q < Q; ++q)
            {
                const T dG = jac_a(q) * w_a(q);
                Cla.noalias() += dG
                    * derivs_a[0].row(q).transpose() * C_a.row(q);
                Clb.noalias() += dG
                    * derivs_a[0].row(q).transpose() * C_b.row(q);
                if (term.value != T(0)) {
                    G.noalias() += (dG * term.value)
                        * derivs_a[0].row(q).transpose();
                }
            }
